...
# ╔═╡ 120db884-4104-4a9a-84d7-7b34a22d0be9
begin
	#-------------------------------------------------------------------------
	# Neural Nets
...
# ╔═╡ 58e851d0-03bc-11ef-12e6-9f9b48146830
md"
=====================================================================================
#### UDL\_20240426\_[3\_2\_ShallowNeuralNetworks\_II](https://github.com/udlbook/udlbook/blob/main/Notebooks/Chap03/3_2_Shallow_Networks_II.ipynb)\_With\_LUX.jl
##### file: UDL\_20240426\_3\_2\_ShallowNeuralNetworks\_II\_With\_LUX.jl
##### code: Julia 1.10.2/Pluto by *** PCM 2024/05/01 ***
=====================================================================================
"

# ╔═╡ 954e8229-877f-4e46-b77c-fd4a4ca2c571
md"
---
##### Packages
"

# ╔═╡ 93fdec02-7155-49c8-a93d-c0d32e1e59f4
md"
---
##### 1. Generation of Data
###### 1.1 Specification of Data Generating Neural Network

Her we reuse the generating network from [UDL\_20240402\_3\_2\_ShallowNeuralNetworks\_II\_With\_Flux.jl](https://uol.de/f/2/dept/informatik/ag/lks/download/Probabilistic_Programming/JULIA/Pluto.jl/Machine_Learning/UnderstandingDeepLearning/UDL_20240402_3_2_ShallowNeuralNetworks_II_With_Flux.html?v=1712487194).
"

# ╔═╡ be204399-b7ae-43d9-bcad-ce6dc3166e15
#------------------------------------------------------------------------------------
# definition of a data generator for Vector{Float64}
#   in the form of a shallow neural network 
#   with, two inputs, three hidden units, and one output
#-------------------------------------------------------------------------------------
# function shallow_2_3_1 with vector parameters x1s, x2s
#
function shallow_2_3_1(x1s::Vector{Float64}, x2s::Vector{Float64}, activationFn, ϕ0, ϕ1, ϕ2, ϕ3, θ10, θ11, θ12, θ20, θ21, θ22, θ30, θ31, θ32)
	...
	#---------------------------------------------------------------------------------
end # function shallow_2_3_1

# ╔═╡ ccc361b1-0e53-4925-96a6-2c5594d12164
#------------------------------------------------------------------------------------
# definition of a data generator for Vector{Float32}
#   in the form of a shallow neural network 
#   with, two inputs, three hidden units, and one output
#-------------------------------------------------------------------------------------
# function shallow_2_3_1 with vector parameters x1s, x2s
#
function shallow_2_3_1(x1s::Vector{Float32}, x2s::Vector{Float32}, activationFn, ϕ0, ϕ1, ϕ2, ϕ3, θ10, θ11, θ12, θ20, θ21, θ22, θ30, θ31, θ32)
	ylims    = (-1, 1)
	#---------------------------------------------------------------------------------
	# linear functions of the input data (Fig 3.8a-c)
	function linearInput(x1s::Vector{Float32}, x2s::Vector{Float32}, θ10, θ11, θ12, θ20, θ21, θ22, θ30, θ31, θ32)
		h1s(x1s, x2s) = θ10 .+ (θ11.*x1s + θ12.*x2s)
		h2s(x1s, x2s) = θ20 .+ (θ21.*x1s + θ22.*x2s)
		h3s(x1s, x2s) = θ30 .+ (θ31.*x1s + θ32.*x2s)
		h1s(x1s, x2s), h2s(x1s, x2s), h3s(x1s, x2s)
	...
	#---------------------------------------------------------------------------------
	function linearInput(x1::Float32, x2::Float32, θ10, θ11, θ12, θ20, θ21, θ22, θ30, θ31, θ32)
		h1(x1, x2) = θ10 + θ11*x1 + θ12*x2
		h2(x1, x2) = θ20 + θ21*x1 + θ22*x2
		h3(x1, x2) = θ30 + θ31*x1 + θ32*x2
		h1(x1, x2), h2(x1, x2), h3(x1, x2)
	...
	wactive1(x1s, x2s) = ϕ1 .* active1(x1s, x2s)
  	...
  	wactive3(x1s, x2s) = ϕ3 .* active3(x1s, x2s)
	#---------------------------------------------------------------------------------
	ys(x1s, x2s) =               # ys is a function (!) needed for contour plots
		ϕ0 .+ (wactive1(x1s, x2s) + wactive2(x1s, x2s) + wactive3(x1s, x2s))
	#---------------------------------------------------------------------------------
	return ys, preactive1, preactive2, preactive3, active1, active2, active3, wactive1, wactive2, wactive3
	#---------------------------------------------------------------------------------
...
# ╔═╡ f4984829-b86e-4cc1-b373-e465a2dbfa8e
md"
---
##### 1.2 Generation of Error-free Training Data in $Array\{Float64,1\}$ Form
"

# ╔═╡ f296eb87-54fe-41a8-994c-9ed71fed3ec5
begin # generation of error-free data with shallow 2-3-1 neural network
	#--------------------------------------------------------------------------------
	# parameters of the neural network generator
	θ10 = -4.0; θ11 =  0.9; θ12 =  0.0
	θ20 =  5.0; θ21 = -0.9; θ22 = -0.5
	...
	ys, rest... =                # ys is a function (!) needed for contour plots
	...
	contour(x1s::Vector{Float64}, x2s::Vector{Float64}, ys, fill=true, xlabel=L"x_1", ylabel=L"x_2", title="Data")
	#--------------------------------------------------------------------------------
end # begin

# ╔═╡ e62eb05c-5c1d-49d8-9393-e5cbb66ae933
# foo is is a binary scalar function for wireframe
function responseSurface(x1s::Vector{Float64}, x2s::Vector{Float64}, zs::Union{Vector{Float32}, Vector{Float64}}, title; foo::Function = (x, y) -> 0.0)
	wireframe(x1s, x2s, foo, xlabel=L"x_1", ylabel=L"x_2", label=L"y=f(x_1, x_2)", title=title)
	scatter!(x1s, x2s, zs, xlabel=L"x_1", ylabel=L"x_2", zlabel=L"y", label=L"data\  point: (x_1, x_2, y)", color=:blue, lw=2)
end # function responseSurface

# ╔═╡ 8b4c8ea2-1d63-4ba3-910b-6549d1df17c6
plot1 = 
	responseSurface(
		x1s, x2s, 
	    # v------------------------------------- # ys is a function (!) of vector args
		ys(x1s, x2s), "Data", 
		foo=(x1, x2) -> ys(x1, x2)) # <-- ys is a scalar binary function for wireframe

# ╔═╡ 3120a2f1-63ac-4991-b702-4718491f1986
md"
###### Generation of dependent variable *yss=ys(x1s, x2s)*
"

# ╔═╡ f60e6113-5584-4ed1-865f-1d7d4a79c60e
yss = ys(x1s, x2s)'                 # ys is a function (!) needed for contour plots

# ╔═╡ 1bdc2a28-d6f0-4290-b992-4b63a2043491
typeof(x1s), typeof(x2s), typeof(yss)

# ╔═╡ 490b6b63-27b6-41ad-a43a-930f2c9a28ad
size(x1s), size(x2s), size(yss)

# ╔═╡ cc929389-8204-49f2-bbb8-67466bb13173
md"
###### Input matrix *xs*
"

# ╔═╡ 1863c382-7f2d-469b-a211-21daa8b90efe
xs = Array{Float32}(undef, (2, 21))

# ╔═╡ 615ecc3d-2f27-4707-bf00-b5c16c51afe8
begin # here we transfer both vectors x1s and x2s into a single input matrix xs
	xs[1,:] = x1s
	xs[2,:] = x2s
end # begin

# ╔═╡ 7860c902-b4f5-46a1-8b07-907d7411dab8
xs

# ╔═╡ f12b0122-a2bf-472f-9f4f-cce1413447c3
md"
---
##### Definition of LUX.jl Network *linear\_2\_3\_1\_Model* with *tanh*-Activation
The *number of parameters* is:

$\;$

$hid \cdot (in+1) + (hid+1) \cdot out = 3 \cdot (2+1) +(3+1) \cdot 1 = 9 + 4 = 13$
...
# ╔═╡ 2aaa743c-f1c4-4b23-9e70-1ecd4b2a0231
linear_2_3_1_Model = Chain(Dense(2 => 3, tanh), Dense(3 => 1))

# ╔═╡ 53b855a2-8045-42a9-aa43-5cf6cc318ba0
md"
---
##### Optimizer: *ADAM (ADAptive Moment Estimation)*
"

# ╔═╡ f2e16630-7f49-4db0-916c-107e34c7de21
begin
	maxEpochs = 3.0E4
	learningRate = 0.03f0 
...
# ╔═╡ a9489f70-76d8-4933-8889-99666183e727
md"
---
##### Loss Function
"

# ╔═╡ 33eac4a0-1c39-4bda-b778-ecb23bfe67e7
function loss_function(model, parameters, states, data)
    xs = data[1]                                       # predictor matrix xs
	ys = data[2]                                       # output vector ys
...
# ╔═╡ 663ca33c-6dfb-43ff-814b-4fdb314dcce6
md"
---
##### [LUX.jl-Training](https://lux.csail.mit.edu/v0.5.16/api/Lux/contrib.html#Lux.Experimental.TrainState)
First we will create a [Lux.Experimental.TrainState](https://lux.csail.mit.edu/dev/api/Lux/contrib#Lux.Experimental.TrainState) which is essentially a convenience wrapper over parameters, states and optimizer states.
...
# ╔═╡ e82e1ca5-def3-43bb-b062-faed772c4290
begin 
	rng = MersenneTwister()
	Random.seed!(rng, 12345)
	trainState = Lux.Experimental.TrainState(rng, linear_2_3_1_Model, optimizer)
end # begin

# ╔═╡ f81edfb5-c8aa-4ba2-944e-5bbb53f6fb3d
# struct ADTypes.AutoZygote
vjp_rule = AutoZygote()                        # vjp = Vector-Jacobian Product Rule

# ╔═╡ 2d9b8f15-7841-4c53-8c69-dda5a1851933
dev_cpu = cpu_device()

# ╔═╡ 74c58981-7a90-4cdf-93ea-e574905ffe4c
dev_gpu = gpu_device()

# ╔═╡ 4b2599a2-d343-434f-a266-a2117000c303
md"
###### Training loop
"

# ╔═╡ 4dc50665-ae9c-4b68-8bb3-ea18044eb6db
...
# ╔═╡ f2fa01cd-8d56-4058-a649-0cdb0188b21c
(xs, yss)

# ╔═╡ 45502d98-7e2a-47e7-a428-a1668c641c9f
trainStateNew, epochs = main(trainState, vjp_rule, (xs, yss), maxEpochs)

# ╔═╡ 166a885a-2a7d-43a8-934e-e2805e572566
md"
###### Predicted values
"

# ╔═╡ 3f77a855-208e-40bb-99f0-6aad124691e8
ysPredNew =
	dev_cpu(Lux.apply(trainStateNew.model, dev_gpu(xs), trainStateNew.parameters, trainStateNew.states)[1])

# ╔═╡ 00b8e59c-e0aa-4803-93ed-6e6dd7f1cb61
plot2 = 
	let myMseLoss = round(mean(abs2.(ysPredNew - yss)), digits=3)
		myCor     = round(cor(yss', ysPredNew')[1], digits=3)
		myCorSq   = round(myCor^2, digits=3)
		wireframe(x1s, x2s, (x1, x2) -> ys(x1, x2), title="Predictions of 2-3-1 NN")
		Plots.scatter!(x1s, x2s, yss', label=L"data\ point\ x \mapsto y",  xlabel=L"x_1", ylabel=L"x_2", zlabel=L"y", color=:blue)
		Plots.plot!(x1s, x2s, ysPredNew[:], label=L"predicted\ \hat y", color=:red, lw=2)
		annotate!(-5.6, -5.0,  -4.0, "mse = $myMseLoss", 7)
		annotate!(-5.6, -5.0,  -8.0, "r(y, y-hat) = $myCor", 7)
		annotate!(-5.0, -5.0, -12.0, "#epochs = $epochs", 7)
	end # let

# ╔═╡ 4c19797b-615d-4ddb-92bd-fa1c74665064
Plots.plot(plot1, plot2)

# ╔═╡ 00e76507-5935-485e-87ae-88b2fe76b4fe
md"
---
##### Summary
We took the mathematical structure of Prince's 2-3-1 shallow neural network as a blueprint for a training data generator implemented in Julia. Then we reconstructed the mathematical 2-3-1-structure in LUX.jl. It was possible to estimate parameters by minimizing the mse so that the model predictions are a nearly exact replication of the y-traing data. The Pearson product-moment correlation between y-training data and y-Hat Flux-model prediction are approximately 0.999 with an mse≈0.043. So these results are even better than those obtained by Flux.jl.
"

# ╔═╡ 5802631b-b67e-46f9-804f-afbe4da186d7
md"
---
##### References
...
- **LUXDL-Doc**; *Fitting a Polynomial using MLP*, [https://lux.csail.mit.edu/dev/tutorials/beginner/2_PolynomialFitting#Fitting-a-Polynomial-using-MLP](https://lux.csail.mit.edu/dev/tutorials/beginner/2_PolynomialFitting#Fitting-a-Polynomial-using-MLP); last visit 2024/04/26

"


# ╔═╡ c2bfe341-15a8-4d06-bee0-09d9807c42b8
md"
====================================================================================
