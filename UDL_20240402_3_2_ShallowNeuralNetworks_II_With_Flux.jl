...
##### code: Julia 1.10.2/Pluto by *** PCM 2024/05/14 ***

===================================================================================
"

# ╔═╡ 0fbd9885-4484-4e2c-8a05-ce94157a42ae
md"
---
##### 0. Introduction
...
The *first* *2-3-1* network is used as a *generator* for training data. The parameter values are taken from a Python [Notebook 3.2 -- Shallow neural networks II](https://github.com/udlbook/udlbook/blob/main/Notebooks/Chap03/3_2_Shallow_Networks_II.ipynb)(Prince, 2024). The *second* *2-3-1* network is used as a classical *statistical model* with unknown parameters to be estimated by minizing the *mse* (mean squares error) between the training data and the model output.
...
# ╔═╡ 6c2b8509-640f-4c40-8011-03cac21c5963
supertypes(Float32)

# ╔═╡ 2adaeb6d-a37d-4ca3-8933-4f021e4953d7
# used only for generator's surface
function responseSurface(
	x1s::Array{AbstractFloat}, 
	x2s::Array{AbstractFloat},
	# foo is a scalar binary function for surface construction in wireframe
	foo::Function;
	title = "") 
	#--------------------------------------------------------------------------------
	wireframe(
		x1s, x2s, foo, xlabel=L"x_1", ylabel=L"x_2", zlabel=L"y", title=title)
	#--------------------------------------------------------------------------------
end # function responseSurface

# ╔═╡ 7a1016e0-fd86-4094-a9c5-09b88235129b
...
function responseSurface(
	...
	#--------------------------------------------------------------------------------
	scatter!(
		x1s, x2s, ys, xlabel=L"x_1", ylabel=L"x_2", zlabel=L"y", label=L"data\  point: (x_1, x_2, y)")
	#--------------------------------------------------------------------------------
end # function responseSurface
...
###### 4.1 Coercion to Array{Float32} and Generation of Dependent Variable 
"

# ╔═╡ bd31c783-f65c-4fcc-aad2-ef623b0ddc58
...
# ╔═╡ 874a9de4-bfa7-4048-b21e-7de84322966c
xsTrain = vcat(x1sTrain, x2sTrain)

# ╔═╡ 01b3c537-2d5e-4eb5-a187-d93900936c28
data = [(xsTrain, ysTrain)]
...
function mySSQ(model::Flux.Chain, xs::Array{Float32}, ys::Array{Float32})::Float32
	#------------------------------------------------------------------------------
...
function myMSE(model::Flux.Chain, xs::Array{Float32}, ys::Array{Float32})::Float32
	#------------------------------------------------------------------------------
...
# ╔═╡ 90610d17-89d2-4de1-9711-be59d2f1470f
md"
---
###### 6.2 Activation Function $a[•]$: e.g. Rectified Linear Unit or ReLU
...
md"
---
###### 6.3 Optimizing Mean-Squares-Error (MSE) by Gradient Descent
"
...
linear_2_3_1_Model, ysHat =
	let 
		...
		# Losses before
		ssqErrorBefore = 
			mySSQ(linear_2_3_1_Model, xsTrain, ysTrain)  # ssqBefore
		mseErrorBefore = 
			myMSE(linear_2_3_1_Model, xsTrain, ysTrain)  # mseBefore
		...
		while ((mseErrorOld - mseErrorNew) >  + δ) 
			...
			mseErrorNew = myMSE(linear_2_3_1_Model, xsTrain, ysTrain)
			epochs += 1
		end # while
		println("epochs = $epochs; mseErrorOld = $mseErrorOld; mseErrorNew = $mseErrorNew")
		#-----------------------------------------------------------------------------
		# model prognosis
		ysHat = linear_2_3_1_Model(xsTrain)
		#-----------------------------------------------------------------------------
		linear_2_3_1_Model, ysHat
	end # let

# ╔═╡ 1e51347c-29bf-4734-8dcc-c1fe731ab46b
x1sTrain, x2sTrain, ysTrain, ysHat

# ╔═╡ f2cbab9c-75e2-44c4-be1b-8be048da28e1
function responseSurface() 
	wireframe(x1s, x2s, (x1, x2) -> 
		linear_2_3_1_Model([convert(Float32,x1), convert(Float32,x2)])[1], xlabel=L"x_1", ylabel=L"x_2", zlabel=L"y", title=L"Model\ Response\ Surface\ & Prediction")
	scatter!(x1sTrain', x2sTrain', ysTrain', mcolor=:cornflowerblue, label=L"data:(x_1,x_2,y)")
	plot!(x1sTrain', x2sTrain', ysHat', color=:red, lw=2, label=L"prediction:(x_1,x_2,\hat y)")
end # function responseSurface

# ╔═╡ af5913fa-d007-4d90-9b30-11a9b1dfe594
plot1 = 
	responseSurface(
		x1s, 
		x2s, 		
		# ys is a scalar binary function for surface construction in wireframe
		foo = ys,
		title = L"Generator's\ Surface\ &\ ... ")

# ╔═╡ 1a92034a-6476-4bb6-8dd3-7eff9578b819
plot2 = 
	responseSurface(
		x1s, 
		x2s, 
...
# ╔═╡ 4f4d73a8-a8c5-4244-a85c-f26d64f639bb
Plots.plot(plot1, plot2)

# ╔═╡ ad163158-3b51-4f23-8265-228be4abc513
let 
	#---------------------------------------------------------------------------------
	plot5 =
		responseSurface(
			x1s, 
			x2s, 
			title = L"Model's\ Surface\ &\ ... ", 
			# ys is a scalar binary function for surface construction in wireframe
			foo = 
				(x1, x2) -> 
					linear_2_3_1_Model([convert(Float32,x1), convert(Float32,x2)])[1])
	#---------------------------------------------------------------------------------
	plot6 = 
		responseSurface(
			x1s, 
			x2s, 
			convert(Vector{Float64}, ysTrain[1,:]),
			title = L"Subspace\ of\ Training Data",
			foo = 
				(x1, x2) -> 
					linear_2_3_1_Model([convert(Float32,x1), convert(Float32,x2)])[1])
...
let 
	#--------------------------------------------------------------------------------
	# Losses after
	ssqErrorAfter = mySSQ(linear_2_3_1_Model, xsTrain, ysTrain)  # ssqAfter
	mseErrorAfter = myMSE(linear_2_3_1_Model, xsTrain, ysTrain)  # mseAfter
	...
	# Plots
	responseSurface()
	annotate!(-6.0, 0.3, - 3.00, "SSQ = $ssqErrorAfter", 6)
	annotate!(-6.0, 0.3, - 7.00, "MSE = $mseErrorAfter", 6)
	annotate!(-6.0, 0.3, -11.00, "r(y, yHat) = $rxy", 6)
end # let
