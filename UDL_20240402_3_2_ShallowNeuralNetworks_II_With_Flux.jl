...
begin
	...
	using Flux, Zygote, ProgressMeter
	#-------------------------------------------------------------------------
	using Flux: train!
	using Statistics
	#-------------------------------------------------------------------------
	# Plotting library
	using Plots, Logistics
	#-------------------------------------------------------------------------
	# LateXify Strings
...
begin # construction of Float32 arrays because Flux assumes just that
	len = length(x1s) 
	#--------------------------------------------------------------------------------
	x1sTrain = Array{Float32}(undef, (1, len))
	x2sTrain = Array{Float32}(undef, (1, len))
	ysTrain  = Array{Float32}(undef, (1, len))
	#--------------------------------------------------------------------------------
	for j in 1:length(x1s)
		x1sTrain[j] = x1s[j]
		x2sTrain[j] = x2s[j]
		ysTrain[j]  = ys(x1s, x2s)[j]
	end # for
	#--------------------------------------------------------------------------------
...
# Sum of Squares Error Loss (equation 2.5)
function mySSQ(model::Flux.Chain, x::Array{Float32}, ys::Array{Float32})::Float32
	#------------------------------------------------------------------------------
	ssq(ŷs, ys) = Flux.Losses.mse(ŷs, ys; agg = sum)
	#------------------------------------------------------------------------------
	ssq(model(x), ys)
end # function mySSQ
...
# Mean Squares Error (equation 2.5)
function myMSE(model::Flux.Chain, x::Array{Float32}, ys::Array{Float32})::Float32
	#------------------------------------------------------------------------------
	mse(ŷs, ys) = Flux.Losses.mse(ŷs, ys; agg = mean)
	#------------------------------------------------------------------------------
	mse(model(x), ys)
end # function myMSE
...
let 
	...
	#--------------------------------------------------------------------------------
	# Plots
	plot(x1sTrain', ysHat', label=L"(x_1, \hat y = ((2=>3,tanh)(3=>1)))", lw=2)
	plot!(x2sTrain', ysHat', label=L"(x_2, \hat y = ((2=>3,tanh),(3=>1)))", lw=2)
	annotate!(0.3,  0.18, "SSQ = $ssqErrorAfter", 12)
